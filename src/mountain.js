/**
 * Mountain World Demo - Three.js + Rapier Physics + Gaussian Splats
 *
 * Experience breathtaking mountain landscapes featuring:
...
const CONFIG = {
	...
	// Assets - Using mountain.spz
	ENVIRONMENT: {
		SPLATS: getAssetUrl(ASSETS.MODELS.MOUNTAIN),
		SPLAT_SCALE: 3,
	},
};
...
async function init() {
	...
	scene.background = new THREE.Color(0x87CEEB); // Sky blue for mountain environment
	...
	// ===== LIGHTING SETUP =====
	// Bright daylight hemisphere lighting for mountain environment
	...
	// Atmospheric point light for mountain ambiance
	...
	splatMesh = new SplatMesh({
		...
		onLoad: () => {
			console.log(`✓ Mountain Gaussian splats loaded (${splatMesh.numSplats} splats)`);
	...
	console.log("🚀 Mountain world initialized successfully!");
}
