/**
 * Ancient World Demo - Three.js + Rapier Physics + Gaussian Splats
 *
 * Experience ancient civilizations and historic landscapes featuring:
...
const CONFIG = {
	...
	// Assets - Using ancient.spz
	ENVIRONMENT: {
		SPLATS: getAssetUrl(ASSETS.MODELS.ANCIENT),
		SPLAT_SCALE: 3,
	},
};
...
async function init() {
	...
	scene.background = new THREE.Color(0xD2B48C); // Sandy beige for ancient environment
	...
	// ===== LIGHTING SETUP =====
	// Warm ancient lighting for historic atmosphere
	const hemiLight = new THREE.HemisphereLight(0xF4E4BC, 0x8B4513, 1.0);
	hemiLight.position.set(0, 20, 0);
	scene.add(hemiLight);

	// Golden hour directional lighting
	const dirLight = new THREE.DirectionalLight(0xFFD700, 0.6);
	dirLight.position.set(15, 25, 10);
	scene.add(dirLight);

	// Atmospheric ancient ambiance light
	const pointLight = new THREE.PointLight(0xDEB887, 1.2, 20);
